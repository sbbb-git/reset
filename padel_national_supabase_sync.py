#!/usr/bin/env python3
"""Sync padel_national_data.json → Supabase (tables padel_clubs, padel_slots).

Calqué sur padel_supabase_sync.py (qui pousse padel_idf_data.json). La
distinction IDF / hors-IDF se fait via la colonne `metro` :
  - sync IDF      → metro = 'idf'
  - sync national → metro = code département (75, 33, ...) ou 'fr' par défaut

Stratégie idempotente :
  - padel_clubs : upsert par slug (clé primaire) — la colonne metro est
    écrasée à chaque sync, donc un club qui passe d'un secteur à l'autre
    sera correctement mis à jour.
  - padel_slots : upsert par id (clé primaire)

Usage :
  - En CI : variables d'env SUPABASE_URL + SUPABASE_SERVICE_KEY définies
  - En local : export SUPABASE_URL=... SUPABASE_SERVICE_KEY=... puis python3 ...
"""
import json
import os
import sys
import time
import urllib.error
import urllib.request

STORE = "padel_national_data.json"
UNIFIED = "padel_club_unified.json"
URL = os.environ.get("SUPABASE_URL", "").rstrip("/")
KEY = os.environ.get("SUPABASE_SERVICE_KEY", "")
BATCH = 500  # nombre de rows par requête PostgREST upsert


def http(path, body, prefer="resolution=merge-duplicates,return=minimal"):
    if not URL or not KEY:
        raise RuntimeError("SUPABASE_URL / SUPABASE_SERVICE_KEY non définis")
    data = json.dumps(body, ensure_ascii=False).encode("utf-8")
    req = urllib.request.Request(
        URL + path, data=data,
        headers={
            "apikey": KEY, "Authorization": f"Bearer {KEY}",
            "Content-Type": "application/json", "Prefer": prefer,
        }, method="POST")
    with urllib.request.urlopen(req, timeout=60) as r:
        return r.status


def upsert(table, rows, on_conflict=None):
    if not rows:
        return 0
    suffix = f"?on_conflict={on_conflict}" if on_conflict else ""
    n_ok = 0
    for i in range(0, len(rows), BATCH):
        chunk = rows[i:i+BATCH]
        for attempt in range(3):
            try:
                http(f"/rest/v1/{table}{suffix}", chunk)
                n_ok += len(chunk)
                break
            except urllib.error.HTTPError as e:
                body = e.read().decode("utf-8", "ignore")[:300]
                print(f"  ⚠️ HTTP {e.code} upsert {table} chunk {i}: {body}", file=sys.stderr)
                if attempt == 2:
                    raise
                time.sleep(2 ** attempt)
    return n_ok


def derive_source(slug, meta):
    return meta.get("source") or (
        "urbanpadel" if slug.startswith("urbanpadel-") else
        "doinsport" if slug.startswith("doinsport-") else
        "playtomic" if slug.startswith("playtomic-") else "anybuddy")


def derive_metro(meta):
    """Renvoie 'idf' pour les clubs IDF, sinon le code dept ou 'fr'.

    Le store national peut contenir un mélange IDF + reste de la France ;
    on garde le marqueur explicite si présent dans meta, sinon on déduit
    de cp/dept.
    """
    m = (meta.get("metro") or "").strip().lower()
    if m:
        return m
    cp = (meta.get("cp") or "").strip()
    dept = (meta.get("dept") or "").strip()
    # IDF = 75, 77, 78, 91, 92, 93, 94, 95
    idf_dept = {"75", "77", "78", "91", "92", "93", "94", "95"}
    if dept and dept[:2] in idf_dept:
        return "idf"
    if cp and cp[:2] in idf_dept:
        return "idf"
    return dept[:2] if dept else "fr"


def main():
    if not os.path.exists(STORE):
        print(f"❌ {STORE} introuvable", file=sys.stderr)
        sys.exit(1)
    store = json.load(open(STORE, encoding="utf-8"))

    # 1. Mapping slug → unified_id (clusters cross-plateforme)
    slug_to_unified = {}
    if os.path.exists(UNIFIED):
        unified = json.load(open(UNIFIED, encoding="utf-8"))
        for cl in unified:
            for m in cl.get("members", []):
                slug_to_unified[m["slug"]] = cl["unified_id"]

    # 2. Préparer rows padel_clubs
    clubs_rows = []
    for slug, b in store.items():
        meta = b.get("meta") or {}
        source = derive_source(slug, meta)
        metro = derive_metro(meta)
        clubs_rows.append({
            "slug": slug,
            "source": source,
            "unified_id": slug_to_unified.get(slug),
            "name": meta.get("name") or slug,
            "cp": meta.get("cp"),
            "city": meta.get("city"),
            "lat": meta.get("lat"),
            "lng": meta.get("lng"),
            "metro": metro,
            "meta": {k: v for k, v in meta.items()
                     if k not in {"name", "cp", "city", "lat", "lng", "source", "slug", "metro"}},
        })

    # 3. Préparer rows padel_slots
    slots_rows = []
    for slug, b in store.items():
        meta = b.get("meta") or {}
        default_source = derive_source(slug, meta)
        for sid, s in (b.get("sessions") or {}).items():
            slots_rows.append({
                "id": f"{slug}|{sid}",
                "club_slug": slug,
                "date": s.get("date"),
                "heure": s.get("heure"),
                "fin": s.get("fin"),
                "duree": s.get("duree"),
                "terrain": s.get("terrain"),
                "court_id": s.get("court_id"),
                "prix": s.get("prix"),
                "statut": s.get("statut"),
                "finie": bool(s.get("finie")),
                "source": s.get("source") or default_source,
                "premier_vu": s.get("premier_vu"),
                "dernier_vu": s.get("dernier_vu"),
            })

    print(f"À syncer (national) : {len(clubs_rows)} clubs, {len(slots_rows)} slots → Supabase")
    n_clubs = upsert("padel_clubs", clubs_rows, on_conflict="slug")
    print(f"  ✅ padel_clubs : {n_clubs} upserts")
    n_slots = upsert("padel_slots", slots_rows, on_conflict="id")
    print(f"  ✅ padel_slots : {n_slots} upserts")
    print("Sync national OK.")


if __name__ == "__main__":
    main()
