#!/usr/bin/env python3
"""Auto-discovery centres Urban Foot IDF — réutilise les détecteurs de
pilates_extension_discover.py sur le catalogue urbanfoot_extension_brands.json.
"""
import json
import os
import sys
...
def main():
    ...
    for key, b in brands.items():
        ...
        res["url"] = url
        resolved[key] = res
        print(f"  ← {res.get('platform')} | {res.get('company_id') or res.get('club_id') or res.get('slug') or 'no-id'}")
    ...
    print(f"\n[URBANFOOT] {n_new} nouveaux + {n_skip} déjà résolus + {n_unknown} unknown / {len([k for k in brands if not k.startswith('_')])} brands")
