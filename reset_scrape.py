...
<div class="wrap">
  ...
  <div class="panel">
    ...
    <p style="color:var(--muted);font-size:12.5px;margin:-4px 0 14px">Compare 2 créneaux jour × tranche (ex. mardi aprem vs samedi matin) : visiteurs, présence moyenne, remplissage.</p>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px" id="creneauCompare"></div>
  </div>

  <div class="panel">
    <h2>🏆 Top 20 créneaux jour × tranche</h2>
    <p style="color:var(--muted);font-size:12.5px;margin:-4px 0 14px">35 buckets (7 jours × 5 tranches : matin / midi / aprem / soirée / fin) classés par visiteurs cumulés.</p>
...
// ============== HEATMAP + COMPARATEUR DE CRÉNEAUX ==============
const TRANCHES_R={matin:{label:'Matin (7-12h)',hours:[7,8,9,10,11]},
...
let CA_R={jour:'Mardi',tranche:'aprem'}, CB_R={jour:'Samedi',tranche:'matin'};
const fillR=t=>t>=0.75?'#5fcf8a':t>=0.5?'#e6c14d':'#e07a6f';
function getFinies(){return rows.filter(r=>r.finie);}
...
function computeCreneauR(jour,tranche){
  const hours=new Set(TRANCHES_R[tranche].hours);
  const f=getFinies().filter(r=>{const h=parseInt((r.heure||'').slice(0,2));return r.jour===jour&&hours.has(h);});
  const presents=f.reduce((s,r)=>s+(r.presents||0),0);
  const capacite=f.reduce((s,r)=>s+(r.capacite||0),0);
  const byAct={};f.forEach(r=>{if(!r.activite)return;byAct[r.activite]=(byAct[r.activite]||0)+(r.presents||0);});
  const topAct=Object.entries(byAct).sort((a,b)=>b[1]-a[1]).slice(0,5);
  ...
  return {n:f.length,presents,capacite,rempl:capacite?presents/capacite:0,moy:f.length?presents/f.length:0,topAct,topCoach};
}
function renderCreneauCompareR(){
  const wrap=document.getElementById('creneauCompare');if(!wrap)return;
  const box=(side,sel)=>{const c=computeCreneauR(sel.jour,sel.tranche);
    ...
    return `<div style="background:var(--card2);padding:14px 16px;border-radius:10px;border-left:3px solid ${color}">
      <div style="display:flex;gap:8px;margin-bottom:12px">
        ...
        <select data-side="${side}" data-field="tranche" style="flex:1;background:var(--card);color:var(--text);border:1px solid var(--line);border-radius:6px;padding:6px 8px;font-size:13px">${Object.entries(TRANCHES_R).map(([k,v])=>`<option value="${k}" ${k===sel.tranche?'selected':''}>${v.label}</option>`).join('')}</select>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:12px">
        ...
        <div><div style="font-size:22px;font-weight:800;color:var(--accent2)">${c.moy.toFixed(1)}</div><div style="font-size:10.5px;color:var(--muted);text-transform:uppercase;letter-spacing:.4px">Visiteurs/séance</div></div>
        <div><div style="font-size:22px;font-weight:800;color:${fillR(c.rempl)}">${Math.round(100*c.rempl)}%</div><div style="font-size:10.5px;color:var(--muted);text-transform:uppercase;letter-spacing:.4px">Remplissage</div></div>
        <div><div style="font-size:22px;font-weight:800;color:var(--accent2)">${nf(c.presents)}</div><div style="font-size:10.5px;color:var(--muted);text-transform:uppercase;letter-spacing:.4px">Visiteurs cumulés</div></div>
      </div>
      <div style="font-size:11px;color:var(--muted);text-transform:uppercase;letter-spacing:.4px;margin-bottom:6px">Top activités · Top coachs</div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:11.5px">
        <div>${c.topAct.length?c.topAct.map(([k,v],i)=>`<div style="padding:3px 7px;background:var(--bg);border-radius:5px;margin-bottom:3px"><span style="color:var(--muted)">${i+1}.</span> ${k.slice(0,20)} <span style="color:var(--accent2);font-weight:700;float:right">${v}</span></div>`).join(''):'<div style="color:var(--muted);font-style:italic">—</div>'}</div>
  ...
  const cA=computeCreneauR(CA_R.jour,CA_R.tranche),cB=computeCreneauR(CB_R.jour,CB_R.tranche);
  const dV=cB.n?Math.round(100*(cA.n-cB.n)/cB.n):0, dM=cB.moy?Math.round(10*(cA.moy-cB.moy))/10:0;
  wrap.innerHTML=`${box('A',CA_R)}${box('B',CB_R)}
    <div style="grid-column:span 2;background:var(--bg);border:1px solid var(--line);border-radius:8px;padding:12px 16px;text-align:center;font-size:13px;color:var(--muted)">
      <b style="color:var(--accent)">${CA_R.jour} ${TRANCHES_R[CA_R.tranche].label}</b> vs <b style="color:var(--gold)">${CB_R.jour} ${TRANCHES_R[CB_R.tranche].label}</b> ·
      Volume : <span style="color:${dV>0?'var(--green)':'var(--red)'};font-weight:700">${dV>0?'+':''}${dV}%</span> ·
      Visiteurs moy. : <span style="color:${dM>0?'var(--green)':'var(--red)'};font-weight:700">${dM>0?'+':''}${dM}</span>
    </div>`;
  wrap.querySelectorAll('select[data-side]').forEach(s=>s.addEventListener('change',e=>{
    const sel=e.target.dataset.side==='A'?CA_R:CB_R;sel[e.target.dataset.field]=e.target.value;
    renderCreneauCompareR();
  }));
}
function renderTopBucketsR(){
  const buckets=[],D=getFinies();
  for(const jour of JOURS){for(const [tk,tv] of Object.entries(TRANCHES_R)){
  ...
  w.innerHTML=buckets.slice(0,20).map((b,i)=>`<div style="display:grid;grid-template-columns:32px 280px 1fr auto;gap:10px;align-items:center;font-size:13px;padding:5px 0;border-bottom:1px solid var(--line)">
    ...
    <span style="height:8px;background:var(--line);border-radius:4px;overflow:hidden"><span style="display:block;height:100%;background:${fillR(b.r)};width:${Math.round(100*b.p/max)}%"></span></span>
    <span style="color:var(--muted);font-variant-numeric:tabular-nums;min-width:160px;text-align:right">${nf(b.p)} visiteurs · ${Math.round(100*b.r)}% remplissage</span>
  </div>`).join('');
}
...
function renderAll(){
  ...
  renderHeatmapR();renderCreneauCompareR();renderTopBucketsR();
}
