#!/usr/bin/env python3
"""Scrap fréquentation Punch Studios (Mindbody via proxy WordPress).

L'API publique https://punch-studios.com/wp-json/mindbody/v1/class expose,
pour une fenêtre glissante de ~7 jours et les 4 studios, les chiffres EXACTS :
- MaxCapacity (capacité), TotalBooked (réservés), TotalSignedIn (présents réels).

Pas d'historique au-delà de la fenêtre côté plateforme -> on accumule chaque
relevé dans punch_data.json (clé = Id du cours). Quand un cours est terminé,
TotalSignedIn est définitif (présence réelle).

Génère : punch_seances.csv et punch.html
"""
import csv
...
def capture():
    ...
    for c in classes:
        ...
        store[cid] = {
            ...
            "lieu": short_loc((c.get("Location") or {}).get("Name")),
    ...
    print(f"{now:%Y-%m-%d %H:%M} : {seen} cours vus, {len(store)} en base ({fin} terminés).")
    return store
...
<title>Punch Studios - Fréquentation</title>
<script>__CHARTJS__</script>
<style>
  :root{--bg:#0c1020;--card:#141a30;--card2:#1d2540;--line:#2a3358;
        --text:#eef1fb;--muted:#97a1c8;--accent:#263fff;--accent2:#6f82ff;
...
<body>
<header>
  <h1>PUNCH STUDIOS &middot; Fréquentation</h1>
...
<div class="wrap">
  ...
  <div class="foot">Source : punch-studios.com (Mindbody). Studios : Champs-Élysées, Louvre, Lafayette, Neuilly.</div>
</div>
<script>
const ALL=__DATA__;
...
function render(){
  ...
  mkChart('cCours',{type:'bar',data:{labels:cours,datasets:[{label:'présents',data:cours.map(c=>byC[c]),backgroundColor:'#6f82ff'}]},
...
document.getElementById('btnExport').addEventListener('click',()=>{
  ...
  const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='punch_seances.csv';document.body.appendChild(a);a.click();a.remove();
});
render();
</script>
</body>
