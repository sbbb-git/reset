#!/usr/bin/env python3
"""Fréquentation Barry's Paris (Mariana Tek).

Tenant : https://barrysbootcamp.marianatek.com — région Paris = "France" (10155).
Planning : GET /api/customer/v1/classes?min_start_date=..&max_start_date=..&region=10155
-> par cours : capacity, available_spot_count, class_type.name, instructors[].name,
location.name / classroom_name, start_datetime, duration, is_cancelled.

Présents = capacity - available_spot_count (tant que capacity>0). Mariana Tek
remet capacity ET available_spot_count à 0 dès qu'un cours a commencé : on fige
donc le dernier décompte connu (avant le début) dans barrys_data.json (clé = id
du cours). Conçu pour tourner souvent (~5 min) afin de capter le remplissage
juste avant chaque séance. CORS fermé -> pas de bouton live (maj côté serveur).
"""
import csv
...
TENANT = "https://barrysbootcamp.marianatek.com"
REGION = 10155  # France (Paris)
...
CONFIG = {
    "key": "barrys", "brand": "BARRY'S PARIS",
    "host": "barrysbootcamp.marianatek.com",
    "store": "barrys_data.json", "html": "barrys.html",
    "csv": "barrys_seances.csv", "price": 20,
    "accent": "#e2231a", "accent2": "#ff6b5e",
}
...
def fetch_classes(start, end):
    out, page = [], 1
    base = f"{TENANT}/api/customer/v1/classes"
    while page <= 20:
        params = urllib.parse.urlencode({
            "min_start_date": start.isoformat(), "max_start_date": end.isoformat(),
            "region": REGION, "page_size": 100, "page": page})
        d = _get(f"{base}?{params}")
        res = d.get("results", [])
        out.extend(res)
...
def capture(cfg):
    ...
    classes = fetch_classes(now.date() - dt.timedelta(days=3),
                            now.date() + dt.timedelta(days=2))
    seen = 0
    for c in classes:
        if c.get("is_cancelled"):
            continue
        iso = c.get("start_datetime") or ""
        if not iso:
            continue
        try:
            sdt = dt.datetime.fromisoformat(iso.replace("Z", "+00:00")).astimezone(PARIS)
        except ValueError:
            continue
        ct = c.get("class_type") or {}
        dur = ct.get("duration") or 0
        edt = sdt + dt.timedelta(minutes=dur)
        cid = str(c.get("id"))
        loc = (c.get("location") or {}).get("name") or ""
        room = c.get("classroom_name") or ""
        lieu = f"{loc} — {room}" if room and room != loc else (loc or room or "Paris")
        coachs = ", ".join(i.get("name", "").strip()
                           for i in (c.get("instructors") or []) if i.get("name"))

        cap = c.get("capacity") or 0
        avail = c.get("available_spot_count")
        # présents = capacité - places libres, uniquement si la capacité est connue
        pres = None
        if cap > 0 and avail is not None:
            pres = max(0, cap - avail)

        prev = store.get(cid) or {}
        # Mariana Tek remet capacity/available à 0 une fois le cours démarré :
        # on conserve alors le dernier décompte connu (figé avant le début).
        if cap <= 0 and prev.get("capacite"):
            cap = prev["capacite"]
            pres = prev.get("presents", 0)
        elif pres is None:
            pres = prev.get("presents", 0)
            cap = cap or prev.get("capacite", 0)
...
<style>
  :root{--bg:#0c0c0e;--card:#16161a;--card2:#212127;--line:#33333d;
        --text:#f5f0f0;--muted:#a89d9d;--accent:__ACCENT__;--accent2:__ACCENT2__;
...
<header>
  ...
  <div class="sub">généré le __GENERATED__ &middot; réservations (Mariana Tek)</div>
</header>
<div class="wrap">
  __META_PANEL__
  <div class="note">ℹ️ <b>Présents</b> = capacité &minus; places restantes, figé juste avant chaque séance. Le décompte <b>s'arrête à la première séance pas encore commencée</b> (les séances à venir, encore vides, sont exclues). Un robot capte le remplissage ~5 min avant chaque séance et l'historique s'accumule jour après jour.</div>
  ...
  <div class="foot">Source : __HOST__ (Mariana Tek) &middot; mise à jour auto ~5 min avant chaque séance.</div>
</div>
<script>
const ALL=__DATA__;
...
Chart.defaults.color='#a89d9d';Chart.defaults.borderColor='#33333d';Chart.defaults.font.family=getComputedStyle(document.body).fontFamily;
const css=v=>getComputedStyle(document.documentElement).getPropertyValue(v).trim();
const ACC=css('--accent')||'#e2231a',ACC2=css('--accent2')||'#ff6b5e';
let charts={};
